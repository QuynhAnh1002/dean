...
class EyeProcessor(VideoProcessorBase):
    ...
    def recv(self, frame):
        ...
        if results.multi_face_landmarks:
            ...
            # 1. Hiệu ứng nhắc chớp mắt (Nhấp nháy góc trái)
            if (curr_time - self.last_blink_time) > 10:
                blink_alpha = (np.sin(curr_time * 7) + 1) / 2
                overlay = img.copy()
                cv2.putText(overlay, "!!! HÃY CHỚP MẮT !!!", (30, 60), 
            ...
            if (curr_time - self.start_time) > WORK_TIME_LIMIT:
                ...
                cv2.putText(img, "NGHỈ MẮT 20 GIÂY!", (w//5, h//2), 
                            cv2.FONT_HERSHEY_DUPLEX, 1.2, (255, 255, 255), 3)

        return frame.from_ndarray(img, format="bgr24")

webrtc_streamer(key="eye-wellness", video_processor_factory=EyeProcessor)
